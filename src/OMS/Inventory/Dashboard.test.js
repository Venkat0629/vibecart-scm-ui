...
global.fetch = jest.fn((url) => {
  if (url === 'http://localhost:8090/vibe-cart/orders/getAllOrders') {
  ...
  if (url === 'http://localhost:8090/vibe-cart/inventory/get-all-inventories') {
    return Promise.resolve({
      ok: true,
      json: () =>
        Promise.resolve([
...
describe('Dashboard Component', () => {
  ...
  test('fetches and processes inventory data correctly', async () => {
    ...
    await waitFor(() => {
        expect(global.fetch).toHaveBeenCalledWith('http://localhost:8090/vibe-cart/inventory/get-all-inventories', {
            method: 'GET',
            headers: { 'Content-Type': 'application/json' },
        });
        // Adjust to the number of calls your component makes
